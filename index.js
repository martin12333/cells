...
CA.prototype.draw = function(afterStep) {
  ...
  resizeCanvasToScreenSize(this.canvas);
  ...
  this.program_copy.use()
    ...
    .uniform('u_height', this.canvas.height)
...
function resizeCanvasToScreenSize(canvas, multiplier) {
...
function main() {
  ...
  window.addEventListener('resize', (event) => {
    // make sure canvas size == screen size
    if(ca.timer)
      return
    ca.draw();
  });
